...
def index():
    ...
    if form.login.is_submitted() and form.login.submit.data:
        hashed_pass_login = sha256_crypt.encrypt(form.login.password.data)

        user = query_db('SELECT * FROM Users WHERE username="{}";'.format(form.login.username.data), one=True)
        if user == None:
            flash('Sorry, this user does not exist!')
        elif sha256_crypt.verify(form.login.password.data,user['password']):
            return redirect(url_for('stream', username=form.login.username.data))
        else:
            flash('Sorry, wrong password!')
    elif form.register.is_submitted() and form.register.submit.data:
        hashed_pass_register= sha256_crypt.encrypt(form.register.password.data)
        ...
        if form.register.password.data == form.register.confirm_password.data:
            if user == None:
                query_db('INSERT INTO Users (username, first_name, last_name, password, secret_question) VALUES("{}", "{}", "{}", "{}","{}");'.format(form.register.username.data, form.register.first_name.data,
                form.register.last_name.data,hashed_pass_register, form.register.secret_question.data))
                return redirect(url_for('index'))
            else:
                flash('Sorry, username is unavailable')
        else: flash("Password and Confirm Password fields must contain the same data")
    elif form.change_pass.is_submitted() and form.change_pass.submit.data:
        user = query_db('SELECT * FROM Users WHERE username="{}";'.format(form.change_pass.username.data), one=True)
        if form.change_pass.password.data == form.change_pass.confirm_password.data:
            if user == None:
                flash("Sorry, this user does not exist!")
            elif user ['secret_question']== form.change_pass.secret_question.data: 
                query_db('UPDATE Users SET password="{}" WHERE username="{}" ;'.format(
                    form.change_pass.password.data,form.change_pass.username.data))
